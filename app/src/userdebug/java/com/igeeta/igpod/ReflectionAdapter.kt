...
class ReflectionAdapter(
    ...
    private val nextObjects: ArrayList<Pair<String, Pair<(() -> ReflectionAdapter)?, (() -> Any?)?>>> =
        arrayListOf()
    ...
    init {
        ...
        if (obj != null) {
            if (obj is List<*>) {
                nextObjects.add(
                    Pair(
                        obj::class.qualifiedName + " with entry count " + obj.size,
                        Pair({ ReflectionAdapter(obj, parent, cb, !listPreview) }, null)
                    )
                )
                for (i in 0..<obj.size) {
                    nextObjects.add(
                        Pair(
                            i.toString() + if (listPreview) (": " + obj[i].toString()) else "",
                            Pair(null) { obj[i] })
                    )
                }
            } else {
    ...
    inner class SimpleVH(itemView: TextView) : RecyclerView.ViewHolder(itemView), OnClickListener {
        init {
