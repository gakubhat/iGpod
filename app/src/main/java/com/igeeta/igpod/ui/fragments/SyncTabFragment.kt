...
/**
 * iGeeta Sync tab. Reuses the sync UI from [com.igeeta.igpod.ui.SyncActivity]
 * so the user can configure and run sync without leaving the main navigation.
 */
class SyncTabFragment : BaseFragment(false) {
    ...
    override fun onCreateView(
        ...
        return inflater.inflate(R.layout.activity_sync, container, false)
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        requireActivity().enableEdgeToEdgeProperly()

        connectionStatus = view.findViewById(R.id.connectionStatus)
        statusDot = view.findViewById(R.id.statusDot)
        appbarLayout = view.findViewById(R.id.appbarlayout)
        // As a bottom-tab, the sync screen needs no title bar or back button.
        ...
        playlistsSynced = view.findViewById(R.id.playlistsSynced)
        serverConfigCard = view.findViewById(R.id.serverConfigCard)
        hostnameInput = view.findViewById(R.id.hostnameInput)
        portInput = view.findViewById(R.id.portInput)
        saveConfigButton = view.findViewById(R.id.saveConfigButton)
        editConfigButton = view.findViewById(R.id.editConfigButton)
        ...
        syncButton.setOnClickListener { startSync() }
        cancelButton.setOnClickListener { cancelSync() }
        cleanButton.setOnClickListener { cleanCollection() }
        saveConfigButton.setOnClickListener { saveConfig() }

        ContextCompat.registerReceiver(
            requireContext(),
            syncReceiver,
            IntentFilter(SyncService.BROADCAST_SYNC_PROGRESS),
    ...
    private fun loadPlaylists() {
        ...
        viewLifecycleOwner.lifecycleScope.launch(Dispatchers.IO) {
            ...
            try {
                ...
                serverPlaylists =
                    playlists.map { IgeetaApi.ServerPlaylistInfo(it.id, it.name, it.description) }
                withContext(Dispatchers.Main) {
    ...
    private fun startSync() {
        ...
        SyncService.start(requireContext(), selectedIds)
    }

    private fun cancelSync() {
        SyncService.cancel(requireContext())
    }
    ...
    inner class PlaylistSelectAdapter :
        ...
        fun selectAll() {
            if (selectedIds.size == playlists.size) selectedIds.clear()
            else selectedIds.addAll(playlists.map { it.id })
            notifyDataSetChanged()
            updateSyncButton()
        }
